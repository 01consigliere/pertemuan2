...
# Header
st.header(':turtle: FISIKA KOMPUTASI AWAN :turtle:')
st.write('anan maulana - 210322607255')
st.subheader('Plot')
...
ax.plot(x, z, label='cos(x)', color='b')  # Plotting sin(x) curve
ax.set_ylabel("")
ax.set_xlabel("x")
...
with col2:
    ...
    fig, ax = plt.subplots(figsize=(16, 8))
    ax.plot(x, y, label='cos(x)', color='b')  # Plotting cos(x) curve
    ax.set_ylabel("Cos x")
    ax.set_xlabel("x")
    ax.tick_params(axis='y', labelsize=20)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=30, ha='right')
...
st.caption('Copyright © Anan 2024')
